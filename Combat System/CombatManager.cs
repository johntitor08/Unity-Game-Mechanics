...
public class CombatManager : MonoBehaviour
{
    ...
    public int combatEnergyRegenPerTurn = 5;
    ...
    [Range(0f, 1f)]
    public float combatEndEnergyFloorPercent = 0.6f;
    ...
    void StartEnemyTurn()
    {
        ...
        StartCoroutine(DelayedCall(enemyActionDelay, ExecuteEnemyAction));
    }
    ...
    private void TryApplyOnHitEffects(EnemyStats target)
    {
        ...
        if (stunEffect != null && UnityEngine.Random.value < stunApplyChance)
    ...
    private int CalculateEnemyDamageToPlayer(int baseDamage, int defense)
    {
        ...
        {
            float d = SettingsManager.Instance.GetDifficulty();

            difficultyMultiplier = d switch
            {
                0 => 0.7f,
                1 => 1.0f,
                2 => 1.3f,
        ...
        int damage = CalculateFinalDamage(baseDamage, defense, false, 0f, difficultyMultiplier);
    ...
    private void EndCombatInternal()
    {
        ...
        int floorEnergy = Mathf.RoundToInt(maxEnergy * combatEndEnergyFloorPercent);

        if (currentEnergy < floorEnergy)
            PlayerStats.Modify(StatType.Energy, floorEnergy - currentEnergy);
